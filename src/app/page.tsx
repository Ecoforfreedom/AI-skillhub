...
async function getHomeData() {
  ...
  for (const s of roleStats) for (const r of s.roleCategories) roleCounts[r] = (roleCounts[r] || 0) + 1
  const catCounts: Record<string, number> = {}
  for (const s of catStats) for (const c of s.functionCategories) catCounts[c] = (catCounts[c] || 0) + 1

  return { totalSkills, newThisWeek, featuredSkills, latestSkills, roleCounts, catCounts }
}

// Stardew-style section header component
function SdvSection({ icon, title, sub, linkHref, linkLabel, children }: {
  icon: string; title: string; sub: string; linkHref: string; linkLabel: string; children: React.ReactNode
}) {
  return (
    <section>
      <div className="flex items-end justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          >{icon}</div>
          <div>
            <h2 className="font-pixel" style={{ fontSize: '11px', color: 'var(--sdv-gold)', textShadow: '1px 1px 0 var(--sdv-sh), 0 0 8px rgba(240,192,48,0.3)' }}>
              {title}
            </h2>
            <p className="font-dot mt-1" style={{ fontSize: '16px', color: 'var(--sdv-dim)' }}>{sub}</p>
          </div>
        </div>
...
export default async function HomePage() {
  ...
  return (
    <div className="pb-20" style={{ position: 'relative', zIndex: 1 }}>
      ...
      {/* ── Hero ── */}
      <section style={{ borderBottom: '4px solid var(--sdv-border)', padding: '56px 0 48px', textAlign: 'center', background: 'linear-gradient(180deg, rgba(42,22,6,0.8) 0%, transparent 100%)' }}>
        <div className="container relative">

          {/* Title sign — like Stardew's stone sign */}
          <div
            ...
            {ROLES.slice(0, 8).map(role => (
              <Link
                ...
                onMouseOver={e => {
                  ...
                  el.style.color = 'var(--sdv-teal)'
                  el.style.borderColor = 'var(--sdv-teal)'
                  el.style.boxShadow = '3px 3px 0 var(--sdv-sh), inset 2px 2px 0 var(--sdv-teal), inset -2px -2px 0 var(--sdv-sh)'
                }}
                onMouseOut={e => {
                  const el = e.currentTarget as HTMLElement
                  el.style.color = 'var(--sdv-warm)'
                  el.style.borderColor = 'var(--sdv-border)'
                  el.style.boxShadow = '3px 3px 0 var(--sdv-sh), inset 2px 2px 0 var(--sdv-hi), inset -2px -2px 0 var(--sdv-sh)'
                }}
              >
                <span>{role.icon}</span>
                <span>{role.name}</span>
        ...
        <div className="container py-4">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              ...
              <div key={i} className="sdv-slot flex items-center gap-3 px-4 py-3">
                <span style={{ fontSize: '22px' }}>{stat.icon}</span>
                <div>
                  <p className="font-pixel" style={{ fontSize: '14px', color: stat.color, textShadow: `0 0 6px ${stat.color}` }}>
                    {stat.value}
                  </p>
                  <p className="font-dot" style={{ fontSize: '15px', color: 'var(--sdv-dim)' }}>{stat.label}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="container mt-12 space-y-16">
        ...
        <SdvSection icon="⭐" title="精选高分工具" sub="综合评分最高、实用性最强" linkHref="/rankings" linkLabel="查看榜单">
          ...
          {featuredSkills.length === 0 && (
            <div
              className="sdv-slot py-16 text-center"
              style={{ border: '4px dashed var(--sdv-border)' }}
            >
            ...
            {CATEGORIES.map(cat => (
              ...
              >
                ...
                <div className="min-w-0">
                  <p className="font-pixel truncate" style={{ fontSize: '6.5px', color: 'var(--sdv-cream)' }}>{cat.name}</p>
                  <p className="font-dot" style={{ fontSize: '14px', color: 'var(--sdv-dim)' }}>{catCounts[cat.id] || 0} 个</p>
                </div>
              </Link>
            ))}
          </div>
