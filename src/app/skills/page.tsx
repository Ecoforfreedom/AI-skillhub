        ...
        <div className="flex flex-col gap-8 xl:flex-row xl:items-end xl:justify-between">
          <div className="max-w-3xl">
            ...
            <p className="font-dot mt-4" style={{ fontSize: '17px', color: 'var(--sdv-dim)', lineHeight: 1.85, maxWidth: 720 }}>
              按岗位、功能和信号强度浏览工具目录。这里不是简单堆链接，而是把工具发现整理成更接近产品评估的决策界面。
            </p>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 min-w-0 xl:w-[520px]">
            {[
              { label: '工具总数', value: total ? String(total) : '...', hint: '持续清洗更新' },
              { label: '排序方式', value: SORT_OPTIONS.find(option => option.value === sortBy)?.label || '推荐评分', hint: '支持多维排序' },
              { label: '当前页', value: String(page), hint: data ? `共 ${data.totalPages} 页` : '正在加载' },
            ].map(item => (
